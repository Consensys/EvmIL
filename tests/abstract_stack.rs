...
fn test_abstract_stack_02() {
    let mut st = AbstractStack::new(0,vec![]);
    assert_eq!(st.push(UNKNOWN).push(UNKNOWN), &AbstractStack::new(2,vec![]));
}

#[test]
fn test_abstract_stack_03() {
    let mut st = AbstractStack::new(0,vec![]);
    assert_eq!(st.push(UNKNOWN).push(UNKNOWN).pop(), &AbstractStack::new(1,vec![]));
}

#[test]
fn test_abstract_stack_04() {
    let mut st = AbstractStack::new(0,vec![]);
    assert_eq!(st.push(UNKNOWN).push(ZERO), &AbstractStack::new(1,vec![ZERO]));
}

#[test]
fn test_abstract_stack_05() {
    let mut st = AbstractStack::new(0,vec![]);
    assert_eq!(st.push(UNKNOWN).push(ZERO).push(ONE), &AbstractStack::new(1,vec![ZERO,ONE]));
}

#[test]
fn test_abstract_stack_06() {
    let mut st = AbstractStack::new(0,vec![]);
    assert_eq!(st.push(UNKNOWN).push(ZERO).push(ONE).pop(), &AbstractStack::new(1,vec![ZERO]));
}

#[test]
fn test_abstract_stack_07() {
    let mut st = AbstractStack::new(0,vec![]);
    assert_eq!(st.push(UNKNOWN).push(ZERO).push(ONE).pop().pop(), &AbstractStack::new(1,vec![]));
}

#[test]
fn test_abstract_stack_08() {
    let mut st = AbstractStack::new(0,vec![]);
    assert_eq!(st.push(UNKNOWN).push(ZERO).push(ONE).pop().pop().pop(), &AbstractStack::empty());
}
...
fn test_abstract_stack_0a() {
    let st = AbstractStack::new(0,vec![ONE,TWO,THREE]);
    assert_eq!(st.set(1,ZERO),AbstractStack::new(0,vec![ONE,ZERO,THREE]));
}
...
fn test_abstract_stack_0f() {
    ...
    assert_eq!(st.set(2,ONE),AbstractStack::new(0,vec![ONE,UNKNOWN,TWO]));
}
...
fn test_abstract_stack_12() {
    ...
    let st2 = AbstractStack::new(1,vec![ONE]);
    assert_eq!(st1.merge(&st2),AbstractStack::new(1,vec![ONE]));
}
#[test]
...
fn test_abstract_stack_14() {
    ...
    let st2 = AbstractStack::new(0,vec![TWO,ONE]);
    assert_eq!(st1.merge(&st2),AbstractStack::new(0,vec![ONETWO,ONE]));
}
#[test]
fn test_abstract_stack_15() {
    let st1 = AbstractStack::new(1,vec![ONE,ONE]);
    let st2 = AbstractStack::new(0,vec![TWO,ONE]);
    assert_eq!(st1.merge(&st2),AbstractStack::new(0..=1,vec![ONETWO,ONE]));
}
#[test]
